        ...
        fetch("./php/respuestas.php", {
            ...
            .then(function(data) {
                ...
                $(data.data).each((index, ele2) => {
                    ...
                    $("#envioResp").append(`<input type="radio" name="${ele2.id_preg}" id="${ele2.id_resp}"> ${modulo}. ${ele2.texto_resp}<br>`);
                });
    ...
    if (aRespUsr.length != aCorrectas.length) {
        ...
        for (let i = 0; i < aCorrectas.length; i++) {
            if (aRespUsr[i] == aCorrectas[i]) { // compara los dos array y si coinciden incrementa acertadas
                ...
                let modulo2 = aCorrectas[i] % 4;

                if (modulo2 == 0) {
                    modulo2 = 4;
                }

                $(`#contRespuestas${i}`).append(`<span class="respErronea">Error, la respuesta correcta es la ${modulo2}</span>`);
            }
        }
